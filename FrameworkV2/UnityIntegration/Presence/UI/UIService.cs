...
// 다중 Realm이 필요하면 SortingLayer 결정부분 수정 필요
    ...
    UIRealmContext CreateContext(Realm realm, string keySuffix = null)
    {
        ...
        return new UIRealmContext
        {
            ...
            PanelRoot = panel,
            ModalRoot = modal,
            NonModalOverlays = new List<GameObject>(),
            ModalOverlays = new List<GameObject>()
        };
    }

    Transform GetParent()
    {
        return uiRootParent != null ? uiRootParent : transform;
    }
    ...
    class UIRealmContext
    {
        ...
        public List<GameObject> NonModalOverlays;
        public List<GameObject> ModalOverlays;
    }
}
